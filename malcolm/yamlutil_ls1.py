        ...
        for k, v in self.param_dict.items():
            for s in substitutions:
                if isinstance(v, str_):
                    ...
                    if not re.search("\$\((" + s + ")\)", v) is None:
                        # If a string
                        if isinstance(substitutions[s], str_):
                            # Straight sub as before
                            v = v.replace("$(%s)" % s, str(substitutions[s]))
                        # If a numeric
                        elif isinstance(substitutions[s], numpy.generic):
                            # Sub the string rep of the value
                            v = v.replace("$(%s)" % s, str(substitutions[s]))
                            # If the last sub in the parameter
                            if "$(" not in v:
                                # Evaluate and return string result (ugly/unsafe atm)
                                v = str(eval(v))
                        # Assume boolean otherwise
                        else:
                            v = substitutions[s]
            param_dict[k] = v
        return param_dict
